...
export default function OrderStatusProgress({
  ...
  if (status === 'cancelled') {
    return (
      <div className={`w-full flex items-center justify-center mb-2 ${className}`}>
  ...
  return (
    <div className={`w-full flex flex-col items-center mb-2 ${className}`}>
      ...
      <div className="relative w-full h-2 bg-secondary-bg rounded-full mb-1">
        <div
          ...
          style={{ width: `${Math.min(((orderStep + 1) / orderSteps.length) * 100, 100)}%` }}
        />
      </div>
    </div>
  );
